...
func TestSyncOnce(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "skips timestamp parsing errors from OCM SDK",
			cluster: newCluster(),
			dbResources: []any{
				newSPC(testManagementClusterResourceID),
			},
...
func TestApplyDesireNeedsWork(t *testing.T) {
	t.Parallel()

	baseDesire := func() *kubeapplierapi.ApplyDesire {
		resourceIDStr := kubeapplierapi.ToClusterScopedApplyDesireResourceIDString(
			testSubscriptionID, testResourceGroupName, testClusterName, "test-resource",
		)
		resourceID := metadataapi.Must(azcorearm.ParseResourceID(resourceIDStr))
		return &kubeapplierapi.ApplyDesire{
			CosmosMetadata: coreapi.CosmosMetadata{
	...
	tests := []struct {
		name     string
		existing *kubeapplierapi.ApplyDesire
		desired  *kubeapplierapi.ApplyDesire
		want     bool
	}{
		{
			name:     "returns true when existing is nil",
			existing: nil,
			desired:  baseDesire(),
			want:     true,
		},
		{
			name:     "returns false when existing matches desired",
			existing: baseDesire(),
			desired:  baseDesire(),
			want:     false,
		},
		{
			name:     "returns true when management cluster differs",
			existing: baseDesire(),
			desired: func() *kubeapplierapi.ApplyDesire {
				d := baseDesire()
				d.Spec.ManagementCluster = metadataapi.Must(azcorearm.ParseResourceID(
					"/providers/microsoft.redhatopenshift/stamps/2/managementclusters/other",
				))
				return d
			}(),
			want: true,
		...
		{
			name:     "returns true when type differs",
			existing: baseDesire(),
			desired: func() *kubeapplierapi.ApplyDesire {
				d := baseDesire()
				d.Spec.Type = kubeapplierapi.ApplyDesireTypeDelete
				return d
			}(),
			want: true,
		},
		{
			name:     "returns true when target item differs",
			existing: baseDesire(),
			desired: func() *kubeapplierapi.ApplyDesire {
				d := baseDesire()
				d.Spec.TargetItem.Name = "different-cm"
				return d
			}(),
			want: true,
		},
		{
			name:     "returns true when KubeContent raw bytes differ",
			existing: baseDesire(),
			desired: func() *kubeapplierapi.ApplyDesire {
				d := baseDesire()
				d.Spec.ServerSideApply.KubeContent = &runtime.RawExtension{Raw: []byte(`{"apiVersion":"v1","kind":"Secret"}`)}
				return d
			}(),
			want: true,
		},
		{
			name: "returns true when existing has nil ServerSideApply and desired has it",
			existing: func() *kubeapplierapi.ApplyDesire {
				d := baseDesire()
				d.Spec.ServerSideApply = nil
				return d
			}(),
			desired: baseDesire(),
			want:    true,
		...
		{
			name:     "returns true when existing has ServerSideApply and desired has nil",
			existing: baseDesire(),
			desired: func() *kubeapplierapi.ApplyDesire {
				d := baseDesire()
				d.Spec.ServerSideApply = nil
		...
		{
			name: "returns false when both ServerSideApply are nil",
			existing: func() *kubeapplierapi.ApplyDesire {
				d := baseDesire()
				d.Spec.ServerSideApply = nil
				return d
		...
		{
			name: "returns true when existing KubeContent is nil and desired is not",
			existing: func() *kubeapplierapi.ApplyDesire {
				d := baseDesire()
				d.Spec.ServerSideApply.KubeContent = nil
				return d
			}(),
			desired: baseDesire(),
			want:    true,
		...
		{
			name: "returns false when both KubeContent are nil",
			existing: func() *kubeapplierapi.ApplyDesire {
				d := baseDesire()
				d.Spec.ServerSideApply.KubeContent = nil
				return d
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			got := applyDesireNeedsWork(tt.existing, tt.desired)
			assert.Equal(t, tt.want, got, "applyDesireNeedsWork result mismatch")
		})
	}
}
