	...
	Context("Connectivity", func() {
		...
		It("should deploy node pool and run workload with authorized CIDRs enabled",
			...
			func(ctx context.Context) {
				...
				By("deploying node pool via SDK")
				nodePoolParams := framework.NewDefaultNodePoolParams()
				nodePoolParams.ClusterName = clusterName
				nodePoolParams.NodePoolName = nodePoolName
				nodePoolParams.OpenshiftVersionId = openshiftControlPlaneVersionId
				...
				Eventually(func() bool {
					cmd := fmt.Sprintf("echo '%s' | base64 -d > /tmp/kubeconfig && kubectl --kubeconfig=/tmp/kubeconfig get nodes --no-headers 2>/dev/null | wc -l", kubeconfigB64)
				...
				createNsCmd := fmt.Sprintf("echo '%s' | base64 -d > /tmp/kubeconfig && kubectl --kubeconfig=/tmp/kubeconfig create namespace test-workload 2>&1 || true", kubeconfigB64)
...
spec:
  ...
  type: ClusterIP`
				...
				deployCmd := fmt.Sprintf("echo '%s' | base64 -d > /tmp/kubeconfig && echo '%s' | kubectl --kubeconfig=/tmp/kubeconfig apply -f - 2>&1",
					kubeconfigB64, escapedYAML)
				...
				Eventually(func() bool {
					cmd := fmt.Sprintf("echo '%s' | base64 -d > /tmp/kubeconfig && kubectl --kubeconfig=/tmp/kubeconfig get pods -n test-workload -l app=nginx --field-selector=status.phase=Running --no-headers 2>/dev/null | wc -l",
						kubeconfigB64)
					output, err := runVMCommand(ctx, tc, *resourceGroup.Name, vmName, cmd)
					if err != nil {
						return false
				...
				testServiceCmd := fmt.Sprintf("echo '%s' | base64 -d > /tmp/kubeconfig && kubectl --kubeconfig=/tmp/kubeconfig run curl-test --image=curlimages/curl:latest --rm -i --restart=Never -n test-workload -- curl -s http://nginx-service.test-workload.svc.cluster.local 2>&1",
					kubeconfigB64)
				...
				cleanupCmd := fmt.Sprintf("echo '%s' | base64 -d > /tmp/kubeconfig && kubectl --kubeconfig=/tmp/kubeconfig delete namespace test-workload --wait=false 2>&1",
					kubeconfigB64)
